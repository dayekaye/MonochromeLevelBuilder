...
public class SuperScreenshot : EditorWindow {
	...
	private void Capture() {
		...
		if (CaptureTransparent) {
			...
			Texture2D captureBlack = new Texture2D(CaptureWidth, CaptureHeight, TextureFormat.RGB24, false);
			...
			CaptureCamera.Render();
			Texture2D captureWhite = new Texture2D(CaptureWidth, CaptureHeight, TextureFormat.RGB24, false);
			captureWhite.ReadPixels(new Rect(0f, 0f, CaptureWidth, CaptureHeight), 0, 0, false);
			captureWhite.Apply();

			// Create capture with transparency
			capture = new Texture2D(CaptureWidth, CaptureHeight, TextureFormat.ARGB32, false);
	...
	private static byte[] Encode(Texture2D texture, Extension extension) {
		if (extension == Extension.TGA) {
			...
			switch (extension) {
				...
				case Extension.EXR: return texture.EncodeToEXR();
